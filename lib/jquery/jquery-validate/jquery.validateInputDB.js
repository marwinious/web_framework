...
(function($) {
	$.fn.extend({
		validateInput: function (options) {
			...
			$(this).find(':input[type=text][data-validate], :input[type=password][data-validate], :input[type=tel][data-validate], textarea[data-validate], :input[type=email][data-validate]').each(function() {
			...
			$(this).submit(function() {
				...
				if(problems == '<ul></ul>' && o.callBack !== '') { return o.callBack(); }
					...
					$('html, body').animate({ scrollTop: scrollOffset.top }, 'slow');
					return false;
				}
			});
			...
			function validate(input,type,problems) {
				...
				for(var c=0;c<multi.length;c++) {
					...
					switch(type) {
						...
						case "email":
							regex = /^([A-Za-z0-9]+)@([A-Za-z0-9]+)\.([A-Za-z0-9]{3})$/;
							found = regex.test(input);
